...
from main.gacalendar.rule_sets import create_rule_set_event, patch_rule_set_event
...
def make_predictions(rule_set_map):
    for rs, transactions in rule_set_map.items():
        transactions = Transaction.objects.filter(
            rule_set=rs).order_by('-date')
...
def update_predictions_from_rule_set(rule_set):
    rule_set_map = defaultdict(list)
    for t in Transaction.objects.filter(rule_set=rule_set).order_by('-date'):
        rule_set_map[rule_set].append(t)
